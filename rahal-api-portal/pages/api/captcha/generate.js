import { generateCaptcha } from '../../../lib/mockStore';

// Generates a simple SVG captcha image (no canvas needed)
function buildSvgCaptcha(code) {
  ...
  let noise = '';
  for (let i = 0; i < 6; i++) {
    noise += `<line x1="${Math.random()*160}" y1="${Math.random()*40}" x2="${Math.random()*160}" y2="${Math.random()*40}" stroke="#${Math.floor(Math.random()*0xffffff).toString(16).padStart(6,'0')}" stroke-width="1" opacity="0.4"/>`;
  }
...
export default function handler(req, res) {
  ...
  const imageBase64 = buildSvgCaptcha(code);
